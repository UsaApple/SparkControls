...
namespace SparkControls.Public
{
    ...
    internal static class MessageBoxEx
    {
        ...
        /// 是否启用动画效果
        /// </summary>
        public static bool EnableAnimate { get; set; } = false;
        ...
        public static DialogResult Show(string message, string caption, string attachMessage = "",
            MessageBoxButtons buttons = MessageBoxButtons.OK, MessageBoxIcon icon = MessageBoxIcon.Information,
            MessageBoxDefaultButton defaultButton = MessageBoxDefaultButton.Button1)
        {
            return Show(null, message, caption, attachMessage, buttons, icon, defaultButton);
        }
        ...
        /// <param name="icon">图标</param>
        /// <param name="defaultButton">默认按钮</param>
        public static DialogResult Show(IWin32Window owner, string message, string caption, string attachMessage = "",
            MessageBoxButtons buttons = MessageBoxButtons.OK, MessageBoxIcon icon = MessageBoxIcon.Information,
            MessageBoxDefaultButton defaultButton = MessageBoxDefaultButton.Button1)
        {
            ...
            return ShowCore(owner, message, caption, attachMessage, buttons, icon, defaultButton);
        }
        ...
        public static DialogResult Show(string message, string caption, Exception exception,
            ...
            MessageBoxDefaultButton defaultButton = MessageBoxDefaultButton.Button1)
        {
            return Show(null, message, caption, exception == null ? string.Empty : exception.ToString(), buttons, icon, defaultButton);
        }
        ...
        /// <param name="icon">图标</param>
        /// <param name="defaultButton">默认按钮</param>
        public static DialogResult Show(IWin32Window owner, string message, string caption, Exception exception,
            MessageBoxButtons buttons = MessageBoxButtons.OK, MessageBoxIcon icon = MessageBoxIcon.Information,
            MessageBoxDefaultButton defaultButton = MessageBoxDefaultButton.Button1)
        {
            return Show(owner, message, caption, exception == null ? string.Empty : exception.ToString(), buttons, icon, defaultButton);
        }

        #endregion

        //内部方法，不检查参数有效性
        private static DialogResult ShowCore(IWin32Window owner, string message, string caption, string attachMessage, MessageBoxButtons buttons, MessageBoxIcon icon, MessageBoxDefaultButton defaultButton)
        {
        ...
        private class MessageForm : SparkFormBase
        {
            /* todo 存在问题：
            ...
            {
                this.button3 = new SparkButton();
                this.txbAttach = new TextBoxUnSelectAllable();
                this.button2 = new SparkButton();
                this.button1 = new SparkButton();
                this.plButtonsZone = new PanelBasic();
                this.ckbToggle = new ToggleButton(this.UseAnimate);
                this.plAttachZone = new PanelBasic();
                this.lbMsg = new MessageViewer();
                ...
                this.Padding = new System.Windows.Forms.Padding(0, 31, 0, 17);
                this.ShowIcon = false;
            ...
            /// 最大默认窗体客户区宽度
            /// </summary>
            readonly int MaxClientWidth = Math.Max(700, Screen.PrimaryScreen.WorkingArea.Width * 2 / 3);
            ...
            private bool UseAnimate { get; set; } = false;

            /// <summary>
            /// 是否启用声音反馈
            /// </summary>
            /// <remarks>此处还弄该属性是为了保证窗体类的独立性</remarks>
            private bool UseSound { get; set; } = true;
                ...
                : this(enableAnimate)
            {
                ...
                //设置关闭按钮是否可以点击
                //只要当OK或包含Cancel可以X,其他不运行X
                if ((Convert.ToInt32(this.MessageButtons) & 1) == 0 && this.MessageButtons != MessageBoxButtons.OK)
                {
                    this.ControlBox = false;//到底是隐藏关闭按钮,还是设置按钮不可用?
                    hideClose = true;
                    this.SetTitleButtonState(TitleItemAction.Close, false);
            ...
            {
                base.OnKeyDown(e);
                if (hideClose && (e.KeyCode == Keys.F4) && (e.Alt == true))  //屏蔽ALT+F4
                {
                    e.Handled = true;
                }
            }
            ...
            {
                if (this.MessageButtons == MessageBoxButtons.OK)
            ...
            {
                return lbMsg.MinimumHeight + plButtonsZone.Height + Padding.Bottom + Padding.Top;
            }
            ...
            private void SetWindowsSetHeight(int increment)
            {
                if (!this.UseAnimate)
                {
                    if (this.Height >= Screen.PrimaryScreen.WorkingArea.Height || this.Height + increment >= Screen.PrimaryScreen.WorkingArea.Height)
                    {
                        //窗口大小设置位最大值
                        this.Height = Screen.PrimaryScreen.WorkingArea.Height;
                        //显示内容缩小
                        lbMsg.Height = lbMsg.Height - increment;
                    }
                    else
                    {
            ...
            private class PanelBasic : Control
            {
                public PanelBasic()
                {
                    ...
                    SetStyle(ControlStyles.AllPaintingInWmPaint, true);//关键，不要设置双缓冲，不然其上的ToolBar不正常
            ...
            private class MessageViewer : Control
            {
                ...
                public override Size GetPreferredSize(Size proposedSize)
                {
                    ...
                    wellSize.Height += 50;//有显示不小的情况,加载一些偏移量
            ...
            /// 屏蔽全选消息的文本框
            /// </summary>
            private class TextBoxUnSelectAllable : SparkTextBox
            {
                protected override void WndProc(ref Message m)
                {
                    //EM_SETSEL
