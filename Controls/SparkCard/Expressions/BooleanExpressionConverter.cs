...
{
	/// <summary>
	/// BooleanExpression 对象的类型转换器实现。
	/// </summary>
	public class BooleanExpressionConverter : TypeConverter
	{
        /// <summary>
		...
		public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
		{
			...
			if (ps.Length != 2 || !bool.TryParse(ps[1], out bool b))
			{
				throw new ArgumentException($"Failed to parse Text({s}) expected text in the format \"Value,Expression.\"");
			}

			//解析字符串并实例化对象
			return new BooleanExpression(b, ps[0].DecodeFromBase64());
		}
		...
		{
			// 将对象转换为字符串
			if ((destinationType == typeof(string)) && (value is BooleanExpression be1))
			{
				return $"{be1.Expression.EncodeToBase64()},{be1.Value}";
			}

			// 生成设计时的构造器代码
			if (destinationType == typeof(InstanceDescriptor) && value is BooleanExpression be2)
			{
				ConstructorInfo ctor = typeof(BooleanExpression).GetConstructor(new Type[] { typeof(bool), typeof(string) });
				return new InstanceDescriptor(ctor, new object[] { be2.Value, be2.Expression });
			}
		...
		{
			bool pv1 = (bool)propertyValues["Value"];
			string pv2 = (string)propertyValues["Expression"];
			return new BooleanExpression(pv1, pv2);
		}
