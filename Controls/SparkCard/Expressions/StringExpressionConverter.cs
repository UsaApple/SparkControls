...
{
	/// <summary>
	/// StringExpression 对象的类型转换器实现。
	/// </summary>
	public class StringExpressionConverter : TypeConverter
    {
        /// <summary>
        ...
        public override object ConvertFrom(ITypeDescriptorContext context, CultureInfo culture, object value)
        {
            ...
            //解析字符串并实例化对象
            return new StringExpression(ps[1].DecodeFromBase64(), ps[0].DecodeFromBase64());
        }
        ...
        {
            // 将对象转换为字符串
            if ((destinationType == typeof(string)) && (value is StringExpression se1))
            {
                return $"{se1.Expression.EncodeToBase64()},{se1.Value.EncodeToBase64()}";
            }

            // 生成设计时的构造器代码
            if (destinationType == typeof(InstanceDescriptor) && value is StringExpression se2)
            {
                ConstructorInfo ctor = typeof(StringExpression).GetConstructor(new Type[] { typeof(string), typeof(string) });
                return new InstanceDescriptor(ctor, new object[] { se2.Value, se2.Expression });
            }
        ...
        {
            string pv1 = (string)propertyValues["Value"];
            string pv2 = (string)propertyValues["Expression"];
            return new StringExpression(pv1, pv2);
        }
