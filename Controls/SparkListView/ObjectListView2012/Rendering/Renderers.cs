...
namespace SparkControls
{
    ...
    public class BaseRenderer : AbstractRenderer
    {
        ...
        public virtual Color GetSelectedBackgroundColor()
        {
            ...
            //{
            //    if (this.ListView.Theme != null)
            //    {
            //        return this.ListView.Theme.SelectedBackColor;
            //    }
            //    else
            //    {
            ...
            //    }
            //}
            if (this.ListView.HideSelection == false || this.ListView.Focused)
            {
                if (this.ListView.Theme != null)
                {
                    return this.ListView.Theme.SelectedBackColor;
        ...
        protected virtual int DrawCheckBox(Graphics g, Rectangle r)
        {
            ...
            if (!DrawCheckBox(g, checkBoxLocation, checkBoxSize, boxState))
            {
        ...
        private bool DrawCheckBox(Graphics g, Point point, Size size, CheckBoxState checkState)
        {
            //if (this.ListView.HotRowIndex > 0 && this.ListItem.Index >= 0)
            //{
            //    Console.WriteLine(this.ListView.HotRowIndex);
            //}
            //

            if (this.ListView.Theme != null)
            {
                RoundRectangle roundRect = new RoundRectangle(point, size, CornerRadius.Empty);
                Color backColor;
                Color borderColor;
                Color tickColor;
                int intCheck = (int)checkState;
                //if (this.ListItem.Selected && intCheck > 4)
                //{
                //    //Check and SelectRow
                //    //组合选中
                //    backColor = this.ListView.Theme.CheckBoxTheme.CombineBackColor;
                //    borderColor = this.ListView.Theme.CheckBoxTheme.CombineSelectedColor;
                //    tickColor = this.ListView.Theme.CheckBoxTheme.CombineSelectedColor;
                //}
                if (intCheck > 4)
                {
                    backColor = this.ListView.Theme.CheckBoxTheme.SelectedBackColor;
                    borderColor = this.ListView.Theme.CheckBoxTheme.SelectedBorderColor;
                    tickColor = this.ListView.Theme.CheckBoxTheme.TickColor;
                }
                else
                {
                    backColor = this.ListView.Theme.CheckBoxTheme.BackColor;
                    borderColor = this.ListView.Theme.CheckBoxTheme.BorderColor;
                    tickColor = this.ListView.Theme.CheckBoxTheme.TickColor;
                }
                ...
                {//第三态
                    ...
                    RectangleF innerRect = roundRect.Rect;
                    innerRect.Inflate(-3, -3);
                    GDIHelper.FillRectangle(g, new RoundRectangle(innerRect, CornerRadius.Empty), tickColor);
                }
                else if (intCheck >= 5)
                {//Check
                    GDIHelper.DrawCheckBox(g, roundRect, backColor, borderColor, 1);
                    GDIHelper.DrawCheckTick(g, roundRect.Rect, tickColor);
                }
                else
                {//UnCheck
                    GDIHelper.DrawCheckBox(g, roundRect, backColor, borderColor, 1);
            ...
            if (il != null)
            {
                ...
                if (selectorAsInt >= 0)
                {
                    if (!this.IsPrinting)
                    {
                        ...
                        NativeMethods2.DrawImageList(g, il, selectorAsInt, r2.X, r2.Y, this.IsItemSelected, !this.ListItem.Enabled);
